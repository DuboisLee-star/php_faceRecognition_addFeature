...
if ($data_inicial && $data_final) {
    ...
    $query = "
    SELECT m.matricula, m.nome, a.validade_craf
    FROM tab_membros m
    JOIN tab_armas a ON m.matricula = a.matricula
    WHERE a.validade_craf BETWEEN '$data_inicial' AND '$data_final'
...
<?php include 'menu_lateral_esq.php';?>
    ...
    <section id="main-content">
      <section class="wrapper">
        <div class="row">
          <div class="col-lg-12">
            <h3 class="page-header"><i class="fa fa-user" aria-hidden="true"></i>Pesquisa em CRAFs</h3>
            <ol class="breadcrumb">
...
<?php if($result): ?>
    <?php foreach($result as $key => $rel): ?>
        <tr>
            ...
            <td><?= date('d/m/Y', strtotime($rel['validade_craf'])); ?></td>
            <td>
                <?php
                // Calcula os dias restantes
                $dataAtual = new DateTime();
                $validade_craf = new DateTime($rel['validade_craf']);
                $interval = $dataAtual->diff($validade_craf);
                $diasRestantes = $interval->format('%R%a dias');
                echo $diasRestantes;
                ?>
            </td>
            <td>
                <?= (strtotime($rel['validade_craf']) >= strtotime('today')) ? '<span class="badge bg-success">válido</span>' : '<span class="badge bg-danger">vencido</span>'; ?>
            </td>
        </tr>
    <?php endforeach; ?>
<?php endif; ?>
