...
// Send E-mail to User
function mailSend($to, $subj, $msg, $att = false, $attn = false, $mime = false) {
	try {
		//$objConfig = new carregaConf ();
		$mail = new PHPMailer (); // create a new object
		$mail->IsSMTP (); // enable SMTP
		$mail->SMTPAuth = true; // authentication enabled
		
		//if (trim ( $objConfig->objportalGeral->mail_sec ) == "X")
		//	$mail->SMTPSecure = 'ssl'; 
		//if (trim ( $objConfig->objportalGeral->mail_sec ) == "T")
			$mail->SMTPSecure = 'tls'; 
		
		$mail->Host = trim ( 'mail.hostmarq.com.br' );
		$mail->Port = trim ( 587 );
		$mail->IsHTML ( true );
		$mail->Username = trim ( 'secretaria@hostmarq.com.br' );
		$mail->Password = trim ( '*Wer3004ber' );
				
		$mail->SetFrom(trim ( 'secretaria@hostmarq.com.br' ),  trim ( 'HOSTMARQ' ));
		$mail->AddReplyTo ( trim ( 'secretaria@hostmarq.com.br' ), trim ( 'HOSTMARQ' ) );
		$mail->Subject = $subj;
		$mail->CharSet = 'ISO-8859-1';
		$mail->AltBody = "Para ver esta mensagem habilite a visualização HTML de seu cliente de e-mail.";
		// $mail->SMTPDebug = 2;
		...
		if($att && $attn){
			$mail->AddStringAttachment($att, $attn, 'base64', $mime);
		}
				if (! $mail->Send ()) {
			return false;
		} else {
