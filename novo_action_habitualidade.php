...
<body>
    <div class='container box-mensagem-crud'>
        ...
        if ($act == 'aprova_habitualidade' && $matricula) {
            ...
            echo '<script>alert("Aprovação realizada com sucesso.");window.location="/atirador/habitualidades.php?id=' . $id . '";</script>';
            exit();
        }
        if ($act == 'excluir_habitualidade' && $matricula && $id_habitualidade_del) {
            ...
            echo '<script>alert("Registro excluido com sucesso.");window.location="/atirador/habitualidades.php?id=' . $id . '";</script>';
            exit();
        }
        ...
        if ($acao == 'editar'):
                ...
                foreach ($habitu_local as $key => $value) {
                    ...
                    if ($matricula) {
						...
						if (strlen(trim($aprovado[$key])) <= 0) {
							if ((int)$clube->tipo_habitualidade == 0) {
								$aprovado[$key] = 1;
								$data_aprovacao[$key] = date('Y-m-d H:i:s');
							} else
						    if ((int)$clube->tipo_habitualidade == 3) {

								// verifica se foi cadastrado dentro do clube
								if ($limite <= 80) {
							...
							if ((int)$clube->tipo_habitualidade == 4 && $limite <= 80) {
								$aprovado[$key] = 0;
								$data_aprovacao[$key] = null;
								$podeCadastrar = true;
							}
                        ...
                        if($podeCadastrar){
               	if(strlen(trim($id_habitualidade[$key])) <= 0){
            ...
            if($clube->biometria == 1){
                ...
                $biometria=0;
    						...
    						$sql = " INSERT INTO tab_habitualidade (matricula, data, id_grupo, local, evento, tipo, modelo, calibre, numsigma, arma_id, qtdemunicoes, livro, datacadastro, municao, tipomunicao, digitado, aprovado,biometria, data_aprovacao) VALUES (:matricula, :data,:id_grupo, :local, :evento, :tipo, :modelo, :calibre, :sigma,:armaId, :qtdemunicoes, :livro, :datacadastro, :municao, :tipomunicao, :digitado, :aprovado,:biometria, :data_aprovacao) ";
    						$stm = $conexao->prepare($sql);
    						$stm->bindValue(':matricula', $matricula);
    					...
    					$stm->bindValue(':municao', (isset($_POST['origem_municao']) ? $_POST['origem_municao'] : ''));
    						$stm->bindValue(':tipomunicao', (isset($_POST['tipo_municao']) ? $_POST['tipo_municao'] : ''));
    						...
    						$stm->bindValue(':datacadastro', $datacadastro);
    						$stm->bindValue(':id', $id_habitualidade[$key]);
            ...
            echo '<script>window.location="habitualidade.php?id=' . $id . '";</script>';
            exit();
        endif;
        ?>
    </div>
