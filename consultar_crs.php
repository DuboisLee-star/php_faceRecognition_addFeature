...
if ($data_inicial && $data_final) {
    // Corrigir a consulta SQL e executar
    $query = "SELECT * FROM tab_membros WHERE validade_cr BETWEEN '$data_inicial 00:00:00' AND '$data_final 23:59:59' ORDER by matricula";
...
<?php include 'menu_lateral_esq.php';?>
    ...
    <section id="main-content">
      <section class="wrapper">
        <div class="row">
          <div class="col-lg-12">
            <h3 class="page-header"><i class="fa fa-user" aria-hidden="true"></i>Pesquisa em CRs</h3>
            <ol class="breadcrumb">
...
<?php if($result): ?>
    <?php foreach($result as $key => $rel): ?>
        <tr>
            ...
            <td><?= $rel['cr']; ?></td>            
            <td><?= date('d/m/Y', strtotime($rel['validade_cr'])); ?></td>
            <td>
                <?php
                // Calcula os dias restantes
                $dataAtual = new DateTime();
                $validade_cr = new DateTime($rel['validade_cr']);
                $interval = $dataAtual->diff($validade_cr);
                $diasRestantes = $interval->format('%R%a dias');
                echo $diasRestantes;
                ?>
            </td>
            <td>
                <?= (strtotime($rel['validade_cr']) >= strtotime('today')) ? '<span class="badge bg-success">válido</span>' : '<span class="badge bg-danger">vencido</span>'; ?>
            </td>
        </tr>
    <?php endforeach; ?>
<?php endif; ?>
