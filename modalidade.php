...
'.PDF_TEXTO.' DECLARA, mediante solicitação de <B>'.$cliente->nome.'</b>, CR <B>'.$cliente->cr.'</b>, está regularmente inscrito nesta entidade sob o nº  <b>'.$cliente->matricula.'</b>, datado de <b>'. ($cliente->data_filiacao ? date('d/m/Y', strtotime($cliente->data_filiacao)) : '') . '</b> e para fim de comprovação junto ao Exército Brasileiro, que promove, realiza ou sedia competições e provas de tiro desportivo, conforme quadro abaixo:<br><br>
...
'.PDF_ASSINA.'
    ...
    $aut = new AutentiqueH();
    $aut->tipo_documento = 'Filiaçãoo'.' '.$cliente->matricula.' '.date('d/m/Y H:i:s');
    $aut->posicao_assinatura   = $dompdf->get_canvas()->get_page_number();
	$aut->criar_documento($documento, $cliente->matricula, true, array(15, 71, 1), array(56, 71, 1)); // (documento, matricula, exibirAssinaturaClube, posicao_assinatura_membro, posicao_assinatura)
	$aut->output();
