...
$id_habitualidade = (isset($_GET['id_habitualidade'])) ? $_GET['id_habitualidade'] : '';
...
if (!empty($id_cliente) && is_numeric($id_cliente)):
	...
	$sql2 = 'SELECT * FROM tab_habitualidade WHERE id ='.$id_habitualidade;
...
<fieldset>
<?php if(empty($habitualidade)):?>
<h3 class="text-center text-danger">Habitualidade não encontrada!</h3>
<?php else: ?>
<form method="post" action="action_habitualidade.php">
...
<div class="form-group">
      <input type="hidden" name="id_habitualidade" value="<?= $value->id; ?>">
<label for="numsigma">Data:</label><br>                       

<input type="datetime" class="form-control mask-date" name="habitu_data[]" maxlength="50" value="<?= date('d/m/Y H:i', strtotime($habitualidade[$idh]->datacadastro)) ?>">
<span class='msg-erro msg-numsigma'></span>
</div>

<div class="form-group">
<label for="tipo">Local:</label><br>
<select class="form-control" name="habitu_local[]" size="1">
    <option value="<?= $habitualidade[$idh]->local ?>"><?= $habitualidade[$idh]->local ?></option>
    <?php foreach ($habitu_op as $op) : ?>
    <option value="<?= $op->local; ?>"><?= $op->local; ?></option>
...
<div class="form-group">
    <label for="calibre">Evento:</label><br>
<select class="form-control" name="habitu_evento[]" size="1">
    <option value="<?= $habitualidade[$idh]->evento ?>"><?= $habitualidade[$idh]->evento ?></option>
    <?php foreach ($habitu_ev as $ev) : ?>
    <option value="<?= $ev->evento; ?>"><?= $ev->evento; ?></option>
    <?php endforeach; ?></select>
...
<div class="form-group">
            ...
            <select name="tipo_<?= htmlspecialchars($idh, ENT_QUOTES, 'UTF-8'); ?>[]" class="form-control">
...
<input type="text" class="form-control" name="habitu_qtdemunicoes[]" size="10" value="<?= $habitualidade[$idh]->qtdemunicoes ?>" id="qtdemunicao">
<span class='msg-erro msg-validade_gt'></span>
</div>

<div class="form-group">
<label for="livro">Livro:</label><br>
<input type="text" class="form-control" name="habitu_livro[]" size="10" value="<?= $habitualidade[$idh]->livro ?>" id="livro">
...
<button type="submit" class="btn btn-primary" id='botao'> Atualizar </button>
