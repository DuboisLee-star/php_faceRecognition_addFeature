...
$sql3 .= " ORDER BY gpa.id ASC, hab.datacadastro ASC, hab.tipo ASC";
...
// Para cada calibre, criar uma tabela separada no PDF
  ...
  <tr><td width="20%" valign="top"  colspan="4"><font size="1">GRUPO DE USO: <b>' . $habitualidades[3]->nome_grupo . ' - '.$habitualidades[3]->descricao.'</b></font></td>
...
$html = '<div align=center style="line-height: 120%; margin-left: 15; margin-right: 15">
</div>
...
$html_habitualidade_com_hash = str_replace('<label>hash</label>',
                                      '<label style="font-size: 12px">Hash SHA256 do original: '.$hash_sha256.'</label>', $html);
...
//Renderizar o html
	...
	$assinante = [
	    "nome" => $assinante->nome,
	    "funcao" => $assinante->funcao,
	    "cpf" => formataCPFCNPJ($assinante->cpf),
	    "cert_path" => $assinante->cert_path,
	    "cert_pass"=> $assinante->cert_pass,
	];
	
	require_once($_SERVER['DOCUMENT_ROOT'].'/icpbrasil/icpbrasil.class.php');
    $icp = new icpBrasil();
    $icp->assinar_documento($documento, $assinante);
    $icp->output();
