...
$mes_corrente = date('m'); // Obtém o mês atual

// Ajustar a consulta para filtrar valores vazios ou nulos na coluna anuidade2024
$result_transacoes = "
    SELECT m.*, f.anuidade2024
    FROM tab_membros AS m
    LEFT JOIN tab_financeiro AS f ON m.matricula = f.matricula
    WHERE m.bloqueio LIKE 'Sim'
    AND YEAR(m.data_renovacao) = $ano_corrente
    AND MONTH(m.data_renovacao) = " . ($mes_corrente - 1) . "
    AND (f.anuidade2024 IS NULL OR f.anuidade2024 = '')
...
<h3 style="text-align: center;">RELA&Ccedil;&Atilde;O DE MEMBROS & ANUIDADES VENCIDAS MÊS ANTERIOR</h3>
...
//Exibibir a página
$dompdf->stream(
    "relacao_membros_anuidade_vencidas",
    array(
