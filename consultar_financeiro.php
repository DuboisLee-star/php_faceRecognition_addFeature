...
$result = false; // Inicializa o resultado para evitar erros
...
if ($data_inicial && $data_final) {
    ...
    // Montar a consulta SQL para buscar nas colunas
       $query = "
    SELECT 
        m.matricula, 
        m.nome, 
    ...
    FROM 
        tab_membros m
    JOIN 
        tab_financeiro_2 as a ON m.matricula = a.matricula
    WHERE 
        (a.data_pgto BETWEEN '$data_inicial' AND '$data_final' OR
         a.data_pgto BETWEEN '$data_inicial' AND '$data_final')
    ORDER BY 
        a.matricula";
...
<?php include 'menu_lateral_esq.php';?>
    ...
    <section id="main-content">
      <section class="wrapper">
        <div class="row">
          <div class="col-lg-12">
            <h3 class="page-header"><i class="fa fa-user" aria-hidden="true"></i>Pesquisar Financeiro</h3>
            <ol class="breadcrumb">
    ...
    <?php if ($result && mysqli_num_rows($result) > 0): ?>
        ...
        <?php while ($rel = mysqli_fetch_assoc($result)): ?>
            <tr>
                ...
                <td><?= $rel['plano']; ?></td>
                <td><?= $rel['valor']; ?></td>
                <td><?= $rel['valor_pago']; ?></td>
                <td>
                    <?php if (empty($rel['valor_pago'])): ?>
